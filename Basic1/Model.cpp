...
GLuint Model::TextureFromFile(const char* name, const std::string& directory)
{
    GLuint texture;
    ...
    unsigned char* data = stbi_load((directory + "/" + std::string(name)).c_str(), &width, &height, &channels, 0);
    if (data)
    {
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
        glGenerateMipmap(GL_TEXTURE_2D);
    }
    else { std::cout << "ERROR::TextureFromFile::stbi_load(): failed to load the " << name << " texture.\n"; return 0; }
